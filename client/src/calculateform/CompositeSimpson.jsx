...
export default function CompositeSimpson() {
  ...
  const handleClick = async (e) => {
    e.preventDefault();
    try {
      await axios.post("http://localhost:8080/composite-simpson", simpson);
  ...
  function calculate() {
    ...
    let I;
    if (N % 2 === 0) {
      // Simpson 1/3 Rule (Composite)
      I = (h / 3) * (f(A) + f(B) + 4 * sumOdd + 2 * sumEven);
    } else {
      // ใช้ Simpson 1/3 จนถึง n-3 แล้วใช้ 3/8 rule สำหรับ 3 ช่องสุดท้าย
      let N3 = N - 3;
      let h1 = (B - A) / N;
      let sumOdd13 = 0;
      let sumEven13 = 0;
      for (let i = 1; i < N3; i++) {
        let x = A + i * h1;
        if (i % 2 === 0) sumEven13 += f(x);
        else sumOdd13 += f(x);
      }

      let I13 = (h1 / 3) * (f(A) + f(A + N3 * h1) + 4 * sumOdd13 + 2 * sumEven13);
    ...
    <div className="trap-container">
      <div className="trap-inputs">
        ...
        <input
          ...
          value={n}
          onChange={handleChange}
          placeholder="Number of intervals (n)"
          type="number"
        />
      </div>
