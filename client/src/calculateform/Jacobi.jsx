...
export default function Jacobi() {
  ...
  const [iterationTable, setIterationTable] = useState([]); // ✅ เก็บค่าทุก iteration
  ...
  // ✅ ฟังก์ชันคำนวณ Jacobi Iteration
  const calculateJacobi = () => {
    try {
      ...
      const n = size;
      let x = Array(n).fill(0); // ค่าตั้งต้น
      ...
      while (iterations < maxIter) {
        ...
        table.push([...xNew]); // ✅ เก็บค่าทุก iteration
  ...
  const handleSave = async () => {
    try {
      await axios.post("http://localhost:8080/jacobi", {
        size,
        matrixA,
        matrixB,
        tolerance,
      });
      alert("Jacobi problem saved successfully!");
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="matrix-container">
      ...
      <div className="buttons">
        <button onClick={calculateJacobi} className="confirm">
          Calculate
        </button>
